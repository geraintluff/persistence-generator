...
class DatabaseBackedCustomClassGenerator {
	public static function phpString($s) {
		return ApiGenerator::phpString($s);
	}
...
class '.$this->className.' extends '.$this->className.$classNameSuffix.' {
	...
	static public function tableStatus() {
		$result = self::querySql("SHOW TABLE STATUS LIKE \'custom_table\'");
		if ($result) {
			return $result[0];
		}
		return $result;
	...
	static public function open($id, $includePending=FALSE) {
		//$result = self::querySql("SELECT FROM custom_table WHERE id=\'".self::escapeString($id)."\'");
		//if ($result) {
	...
	protected function save($create=FALSE, $forceId=NULL) {
		if ($create) {
			//$result = self::querySql("INSERT INTO custom_table ????");
		} else {
			//$result = self::querySql("UPDATE custom_table SET");
		}
		if (!$result) {
			// throw an error
		}
	...
	public function delete() {
		parent::delete();
		return self::querySql("DELETE FROM custom_table WHERE id=\'".self::escapeString($this->id())."\'");
	}
}
?>';
	}
...
class '.$this->className.'_pending'.$classNameSuffix.' extends '.$pendingClass.' {
	...
	public function open() {
		return '.$this->className.'::open($this->id);
	}
}

abstract class '.$this->className.$classNameSuffix.' extends '.$commonClass.' implements IteratorAggregate {
	static private $cache = array();
	static private $pendingIds = array();
	...
	static public function open($id, $includePending=FALSE) {
		return '.$this->className.'::open($this->id, $includePending);
	}
	...
	public function delete() {
		...
		foreach ($this as $key => $value) {
			if ($value instanceof '.$commonClass.' || $value instanceof '.$commonArrayClass.') {
				$value->delete();
			}
		}
		return $result;
